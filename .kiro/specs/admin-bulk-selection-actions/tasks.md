...
- [ ] 1. Establish shared domain contracts and persistence foundations
  - [ ] 1.1 Create immutable DTOs/value objects for `BulkCommand`, `BulkItemResultData`, `BulkResultData`, `Filter_Context`, session edit/notes responses, calendar events and room options.
  ...
  - [ ] 1.2 Add the additive `audit_records` migration, model and privacy-allowlisted `AuditRecordService`.
  ...
  - [ ] 1.3 Add isolated factories/fixtures and generators for teachers, students, sessions, enrollments, rooms, protected dependencies, relation conflicts and policy actors used by unit and feature tests.
...
- [ ] 2. Implement selection context and bulk backend execution
  - [ ] 2.1 Implement `SelectionContextService` plus teacher/student list query services and `BulkRowViewData`.
  ...
  - [ ] 2.2 Implement `BulkPreviewRequest`, `BulkRequest`, named preview/execution routes and thin `AdminBulkActionController` endpoints.
  ...
  - [ ] 2.3 Implement `BulkAuthorizationService` using `viewAny` and per-record `update`/`delete` Policy/Gate checks.
  ...
  - [ ] 2.5 Implement `BulkResultResource` and localized result/error payloads.
  ...
  - [ ] 2.6 Wire accepted and rejected bulk requests to exactly one execution or rejected-operation audit event after processing, including privacy filtering and aggregate/item outcomes.
  ...
  - [ ]* 2.12 Add focused bulk unit/feature/security tests for routes, CSRF/auth, context preview, dependency checks, per-item transactions, partial outcomes and audit privacy.
...
- [ ] 3. Build the reusable teacher/student bulk-selection UI
  - [ ] 3.1 Integrate `toolbar.blade.php`, `confirmation-dialog.blade.php` and `result-summary.blade.php` into the existing teacher and student list views.
  ...
  - [ ] 3.2 Implement the Alpine selection state machine for current-page selection, entity/filter/sort/page invalidation, refresh/logout clearing and non-persistent browser-session behavior.
  ...
  - [ ] 3.3 Implement explicit all-filtered preview, server-resolved count, delete confirmation, exactly-once submission, pending state, recovery actions and accessible live result/error announcements.
