...
<head>
    ...
    <title>{{ $title ?? 'پنل مدیریت پارسیان' }}</title>
...
<body class="relative min-h-screen overflow-x-hidden bg-gray-950 text-gray-100 antialiased">
    ...
    <div class="pointer-events-none fixed top-1/4 right-1/4 z-0 h-80 w-80 rounded-full bg-amber-500/[0.04] blur-[120px]"></div>
    <div class="pointer-events-none fixed bottom-0 left-1/4 z-0 h-72 w-72 rounded-full bg-amber-400/[0.03] blur-[100px]"></div>
    ...
    <div class="relative z-10 flex min-h-screen">
        ...
        <aside
            ...
            <div class="relative flex h-20 items-center gap-2.5 overflow-hidden border-b border-gray-800/60 px-6">
                <div class="pointer-events-none absolute -right-4 top-1/2 h-24 w-24 -translate-y-1/2 rounded-full bg-amber-500/[0.08] blur-2xl"></div>
                ...
                <span class="relative text-lg font-semibold text-amber-100">پارسیان موزیک</span>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-5">
                <ul class="space-y-1">
                    ...
                    {{-- Students --}}
                    <li>
                        <a href="{{ route('admin.students.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-300 transition hover:bg-gray-800/50 hover:text-amber-300">
                    ...
                    {{-- Teachers --}}
                    <li>
                        <a href="{{ route('admin.teachers.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-300 transition hover:bg-gray-800/50 hover:text-amber-300">
                    ...
                    {{-- Sessions / Classes --}}
                    <li>
                        <a href="{{ route('admin.sessions.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-300 transition hover:bg-gray-800/50 hover:text-amber-300">
                    ...
                    {{-- Calendar / Schedule --}}
                    <li>
                        <a href="{{ route('admin.calendar.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-300 transition hover:bg-gray-800/50 hover:text-amber-300">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-amber-400/80" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            تقویم
                        </a>
                    </li>
                    ...
                    {{-- Reports --}}
                    <li>
                        <a href="{{ route('admin.reports.attendance') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-gray-300 transition hover:bg-gray-800/50 hover:text-amber-300">
            ...
            <div class="border-t border-gray-800/60 px-6 py-4">
                <p class="text-xs text-gray-500">آکادمی پارسیان موزیک</p>
            </div>
        </aside>

        {{-- Main area --}}
        <div class="flex flex-1 flex-col lg:pr-64">
            ...
            <header class="relative sticky top-0 z-20 flex h-20 items-center justify-between overflow-hidden border-b border-gray-800/60 bg-gray-950/80 px-6 backdrop-blur-md">
                ...
                <div class="relative">
                    <h2 class="text-sm font-medium text-gray-400">{{ $pageTitle ?? 'داشبورد' }}</h2>
                </div>
