...
<form method="POST" action="{{ route('admin.teachers.update', $teacher) }}" class="max-w-2xl space-y-5"
      x-data="adminDateForm('hire_date', '{{ old('hire_date', $teacher->hire_date?->format('Y-m-d') ?? '') }}')">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input type="text" name="full_name" {{ feedback_field_attributes('full_name') }} value="{{ old('full_name', $teacher->full_name) }}" required
    ...
    <div>
        ...
        <input type="tel" name="phone" {{ feedback_field_attributes('phone') }} value="{{ old('phone', $teacher->phone) }}" required
               class="block w-full rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-3 text-sm text-gray-100 placeholder-gray-500 transition focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20"
    ...
    <div>
        ...
        <input type="hidden" name="hire_date" {{ feedback_field_attributes('hire_date') }} :value="isoValue">
    ...
    <div class="flex items-center gap-4 pt-2">
        <x-admin.submit-button :label="__('admin.update_teacher')" />
