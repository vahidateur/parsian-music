        ...
        <table class="w-full text-sm" role="table" aria-label="لیست کاربران">
            ...
            <tbody class="divide-y divide-gray-800/40">
                ...
                <tr class="group transition duration-150 hover:bg-gray-800/25">
                    ...
                    <td class="px-4 py-3">
                        @php
                            // An account is never managed by its own owner from this list,
                            // which is exactly what the controller enforces server-side.
                            $manageable = $user->id !== $actorId
                                && \Illuminate\Support\Facades\Gate::any(['assign', 'toggle', 'resetPassword', 'delete'], $user);
                        @endphp
                        @if ($manageable)
                        <div class="flex items-center justify-center gap-1">
                            {{-- Edit (the form assigns the role, so it follows the assign ability) --}}
                            @can('assign', $user)
                            ...
                            <form method="POST" action="{{ route('admin.users.toggle', $user) }}" class="inline">
                                ...
                                <button type="submit"
                                        ...
                                        aria-label="{{ $user->is_active ? 'غیرفعال‌سازی' : 'فعال‌سازی' }}"
                            ...
                            <button type="button"
                                    x-on:click="$dispatch('open-modal', 'confirm-user-reset-{{ $user->id }}')"
                                    ...
                                    aria-label="ریست رمز {{ $user->full_name }}"
                                    title="ریست رمز">
                            ...
                            <x-modal name="confirm-user-reset-{{ $user->id }}" variant="confirmation"
                                     :entity="$user->full_name"
                                     :action="__('admin.reset_password')"
                                     :consequence="__('admin.confirmation_consequence_reset_password')">
                                <x-admin.form-state>
                            ...
                            <x-modal name="confirm-user-delete-{{ $user->id }}" variant="confirmation"
                                     :entity="$user->full_name"
