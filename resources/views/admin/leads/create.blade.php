...
<div class="mb-8">
    <h1 class="text-2xl font-semibold text-amber-100">{{ __('admin.create_lead') }}</h1>
    <p class="mt-1 text-sm text-gray-500">{{ __('admin.register_lead_desc') }}</p>
</div>
...
<x-admin.form-state>
<form method="POST" action="{{ route('admin.leads.store') }}" class="max-w-2xl space-y-5">
    @csrf
    @include('admin.leads.partials.form-fields', compact('instruments', 'teachers', 'assignees') + ['lead' => null])

    <div class="flex items-center gap-4 pt-2">
        <x-admin.submit-button :label="__('admin.create_lead')" :class="$btnPrimary" />
