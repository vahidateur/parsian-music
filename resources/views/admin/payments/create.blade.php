...
<div class="mb-8">
    <h1 class="text-2xl font-semibold text-amber-100">{{ __('admin.create_payment') }}</h1>
</div>
...
<form method="POST" action="{{ route('admin.payments.store') }}" class="max-w-2xl space-y-5">
    @csrf
    ...
    <div>
        ...
        <select name="student_enrollment_id" required
            ...
            @foreach ($enrollments as $enrollment)
                <option value="{{ $enrollment->id }}" {{ old('student_enrollment_id') == $enrollment->id ? 'selected' : '' }}>
    ...
    <div>
        ...
        <input type="number" name="amount_total" value="{{ old('amount_total') }}" required step="any" min="0"
               class="block w-full rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-3 text-sm text-gray-100 transition focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
        @error('amount_total')
            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <input type="number" name="discount" value="{{ old('discount', 0) }}" step="any" min="0"
               class="block w-full rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-3 text-sm text-gray-100 transition focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
        @error('discount')
            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <input type="number" name="amount_paid" value="{{ old('amount_paid') }}" required step="any" min="0"
               class="block w-full rounded-lg border border-gray-700 bg-gray-800/50 px-4 py-3 text-sm text-gray-100 transition focus:border-amber-500/50 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
        @error('amount_paid')
            <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
        @enderror
    ...
    <div>
        ...
        <select name="payment_method" required
            ...
            @foreach (\App\Enums\PaymentMethodEnum::values() as $method)
                <option value="{{ $method }}" {{ old('payment_method') === $method ? 'selected' : '' }}>{{ __('admin.payment_methods.' . $method) }}</option>
            @endforeach
        </select>
